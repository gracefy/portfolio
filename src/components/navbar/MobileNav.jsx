  ...
  return (
    <div className="md:hidden">
      <button onClick={() => setIsOpen(!isOpen)} className="p-2 rounded-md">
        {isOpen ? (
          <IoClose className="h-6 w-6 text-primaryBlue dark:text-gray-300" />
        ) : (
          <IoMenu className="h-6 w-6 text-primaryBlue dark:text-gray-300" />
        )}
      </button>
        ...
        <ul className="absolute top-16 left-0 w-full bg-gray-100 dark:bg-gray-900 shadow-lg p-4 space-y-2 font-semibold">
          {links.map((link) => (
            <li key={link.section} className="group">
              <a
                href={`#${link.section}`}
                className="block text-lg text-center text-primaryBlue dark:text-gray-300 hover:text-lime-600 dark:hover:text-lime-400 transition-all duration-500"
              >
                {link.name}
              </a>
